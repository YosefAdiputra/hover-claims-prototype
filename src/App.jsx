  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900 antialiased">
      <style>{`
        ...
        .font-sans-ui { font-family: 'Geist', ui-sans-serif, system-ui, sans-serif; }
        .font-mono-ui { font-family: 'Geist Mono', ui-monospace, monospace; }
        html, body, #root { font-family: 'Geist', ui-sans-serif, system-ui, sans-serif; }
        .tabular { font-variant-numeric: tabular-nums; }
        .grid-bg {
          background-image: linear-gradient(rgba(120,113,108,0.06) 1px, transparent 1px),
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-stone-900 rounded-md flex items-center justify-center">
              <Hexagon className="w-4 h-4 text-emerald-400" strokeWidth={2.5} />
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className="font-semibold text-stone-900 tracking-tight">Hover</span>
              <span className="text-[11px] text-stone-500 uppercase tracking-wider font-medium">Claims</span>
            </div>
          </div>
          <nav className="flex items-center gap-1.5 text-[13px] text-stone-500">
            {crumbs.map((c, i) => (
              <span key={i} className="flex items-center gap-1.5">
                {i > 0 && <ChevronRight className="w-3 h-3 text-stone-300" />}
                <span className={i === crumbs.length - 1 ? 'text-stone-900 font-medium' : ''}>{c}</span>
              </span>
            ))}
          </nav>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-stone-100 rounded-md text-[12px] text-stone-600">
            <Search className="w-3.5 h-3.5" />
            <span>Search claims</span>
            <kbd className="ml-2 px-1.5 py-0.5 bg-white border border-stone-200 rounded text-[10px] font-mono-ui text-stone-400">⌘K</kbd>
          </div>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-stone-700 to-stone-900 flex items-center justify-center text-white text-[11px] font-medium">YD</div>
        </div>
      </div>
    </header>
  );
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[11px] uppercase tracking-wider text-stone-500 font-medium">My Queue</span>
            <span className="text-[11px] text-stone-400">•</span>
            <span className="text-[11px] text-stone-500">Wednesday, April 8, 2026</span>
          </div>
          <h1 className="font-display text-5xl text-stone-900 leading-none">Good morning, Yosef.</h1>
          <p className="text-stone-500 mt-3 text-[14px]">You have <span className="text-stone-900 font-medium">6 claims</span> in your queue. 3 are ready to review with high-confidence AI drafts.</p>
        </div>
        <div className="flex items-center gap-2">
          <button className="text-[13px] px-3 py-1.5 text-stone-600 hover:text-stone-900">Filter</button>
          <button className="text-[13px] px-3 py-1.5 text-stone-600 hover:text-stone-900">Export</button>
        </div>
      </div>
        ...
        {DASHBOARD_CLAIMS.map((c, idx) => (
          ...
          >
            ...
            <div>
              ...
              {c.confidence && (
                <div className="text-[11px] text-stone-500 mt-1 tabular">{c.confidence}% confidence</div>
              )}
            </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex items-center gap-2 text-[11px] text-stone-500">
              <Shield className="w-3 h-3" /> Submitted as Yosef Durr, Adjuster ID ADJ-4421 · {new Date().toLocaleDateString()}
            </div>
          </div>
        </div>
      </div>
